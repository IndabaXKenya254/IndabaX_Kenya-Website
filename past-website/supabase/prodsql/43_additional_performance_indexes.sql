...
-- Note: Removed "expires_at > NOW()" from WHERE clause because NOW() is not IMMUTABLE
...
'Index for review locks. Query should filter expires_at > NOW() at runtime. Critical for preventing concurrent edits by multiple reviewers.';
