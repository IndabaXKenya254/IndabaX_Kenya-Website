...
import { convertHeicToJpeg, isHeicFile, isVideoFile } from './heic-converter'
...
export async function uploadFile(
  ...
  try {
    ...
    let storagePath = generateStoragePath(file.name)
    ...
    // Handle HEIC conversion (check both MIME type and file extension)
    if (isHeicFile(file.type, file.name)) {
      console.log('HEIC file detected, converting to JPEG...')
      const conversionResult = await convertHeicToJpeg(buffer, 90)

      if (!conversionResult.success || !conversionResult.buffer) {
        return {
          success: false,
          error: conversionResult.error || 'Failed to convert HEIC image',
        }
      }

      buffer = Buffer.from(conversionResult.buffer)
      finalMimeType = 'image/jpeg'
      // Update storage path to use .jpg extension
      storagePath = storagePath.replace(/\.(heic|heif)$/i, '.jpg')
      console.log('HEIC conversion completed successfully')
    }

    // Skip image processing for videos
    if (isGalleryPhoto && isVideoFile(finalMimeType)) {
      console.log('Video file detected, uploading without processing...')
      ...
      if (!publicUrlData || !publicUrlData.publicUrl) {
        return {
          success: false,
          error: 'Failed to generate public URL for uploaded video',
        }
      }
