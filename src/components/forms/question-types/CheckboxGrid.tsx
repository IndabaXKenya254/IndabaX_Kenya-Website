...
// ═══════════════════════════════════════════════════════════════════════
// INDABAX KENYA - CHECKBOX GRID QUESTION TYPE
// ═══════════════════════════════════════════════════════════════════════
// Grid with checkboxes (multiple selections per row)
...
export function CheckboxGrid({ question, value = {}, onChange, onUpdate, mode }: CheckboxGridProps) {
  ...
  const handleCellChange = (row: string, col: string) => {
    const currentRowValues = value[row] || []
    const newRowValues = currentRowValues.includes(col)
      ? currentRowValues.filter(v => v !== col)
      : [...currentRowValues, col]

    const newValue = { ...value, [row]: newRowValues }
    onChange?.(newValue)
  }
  ...
  const hasSelections = Object.values(value).some(arr => arr && arr.length > 0)
              ...
              <tr key={rowIndex}>
                ...
                {columns.map((col: string, colIndex: number) => (
                  <td key={colIndex} className="text-center">
                    <input
                      type="checkbox"
                      checked={(value[row] || []).includes(col)}
