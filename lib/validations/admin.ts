...
export const createPostSchema = z.object({
  ...
  // NEW: Featured flag and author display fields
...
export const createEventSchema = z.object({
  ...
  status: z.enum(['draft', 'published', 'archived', 'upcoming']).default('draft'),
  event_type: z.enum(['upcoming', 'past', 'workshop', 'conference', 'meetup']).default('upcoming'),
...
export const updateEventSchema = z.object({
  ...
  status: z.enum(['draft', 'published', 'archived', 'upcoming']).optional(),
  event_type: z.enum(['upcoming', 'past', 'workshop', 'conference', 'meetup']).optional(),
  registration_url: z.string().url('Invalid registration URL').optional().or(z.literal('')).nullable(),
  max_attendees: z.number().int().positive().optional().nullable()
...
export const createSpeakerSchema = z.object({
  ...
  country: z.string().max(100).trim().optional(), // NEW: Speaker country
  is_featured: z.boolean().default(false),
...
export const createSponsorSchema = z.object({
  ...
  description: z.string().max(1000).trim().optional(), // NEW: Sponsor description
  display_order: z.number().int().min(0).default(0),
...
export const createFaqSchema = z.object({
  ...
  category: z.enum(['registration', 'venue', 'schedule', 'speakers', 'general']).optional(),
  display_order: z.number().int().min(0).default(0),
  is_active: z.boolean().default(true),
})
