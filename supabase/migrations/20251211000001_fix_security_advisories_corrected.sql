-- Migration: Fix Security Advisories from Supabase Linter (CORRECTED)
-- Date: 2025-12-11
...
ALTER FUNCTION check_in_ticket(p_ticket_id uuid, p_checked_in_by uuid) SET search_path = public, auth;
ALTER FUNCTION lookup_ticket_by_qr(p_qr_data text) SET search_path = public, auth;

-- 2.2 Review lock functions
ALTER FUNCTION cleanup_expired_locks() SET search_path = public, auth;
ALTER FUNCTION acquire_review_lock(p_registration_id uuid, p_user_id uuid, p_ip_address character varying, p_lock_duration_minutes integer) SET search_path = public, auth;
ALTER FUNCTION release_review_lock(p_registration_id uuid, p_user_id uuid, p_force boolean) SET search_path = public, auth;
ALTER FUNCTION is_application_locked(p_registration_id uuid) SET search_path = public, auth;
ALTER FUNCTION get_reviewer_workload(p_event_id uuid) SET search_path = public, auth;
...
ALTER FUNCTION get_user_role(user_id uuid) SET search_path = public, auth;
...
ALTER FUNCTION generate_resume_token() SET search_path = public, auth;
...
ALTER FUNCTION log_application_activity(p_application_id uuid, p_activity_type character varying, p_user_id uuid, p_user_email character varying, p_details text, p_metadata jsonb) SET search_path = public, auth;
...
BEGIN
  ...
  SELECT COUNT(*) INTO fixed_functions_count
  FROM pg_proc p
  ...
  RAISE NOTICE '   - Views recreated with SECURITY INVOKER: 2 (applications_with_locks, reviewer_stats)';
  RAISE NOTICE '   - Functions with search_path set: % / 18', fixed_functions_count;
  ...
  ELSE
    RAISE WARNING '⚠️  pg_trgm is in % schema', extension_schema;
  END IF;
END $$;
